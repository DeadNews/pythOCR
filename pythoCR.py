...
def analyse_word_count(sub_data, language):
    word_count = dict()
    enchant_dict = enchant.Dict({"eng": "en_US", "fra": "fr_FR", "rus": "ru_RU"}[language])
...
def user_input_replace_confirm(word, substitutes, fullstring):
    ...
    msg = "Dialogue: \"%s\"\nBad word found, please select a substitute or enter [s] to skip:\n" % strip_tags(
        displaystring)
    msg += ", ".join([("\"%s\"[" + Fore.BLUE + Style.BRIGHT + "%d" + Style.RESET_ALL + "]") %
...
def extreme_try_word_without_char(word, fullstring, chars_to_try_to_replace, enchant_dict, word_count):
    if enchant_dict.check(word):
        ...
        substitutes = [(substitute, difflib.SequenceMatcher(None, word, substitute).ratio())
                       for substitute in list(set(substitutes)) if enchant_dict.check(substitute)]
        if len(substitutes) > 0:
            logging.debug("Heuristic - Found bad word \"%s\", possibles substitutes are \"%s\"" %
                          (word, str(substitutes)))
            if args.timid:
                return user_input_replace_confirm(word, substitutes, fullstring)
            else:
                chosen_subst = sorted(substitutes,
...
def extreme_try_string_without_char(string, chars_to_try_to_replace, enchant_dict, word_count):
    for word in re.findall(r"\w+[" + re.escape("".join([char[0] for char in chars_to_try_to_replace])) + r"]\w+", string,
                           flags=re.UNICODE):
...
def extreme_try_subs_without_char(sub_data, chars_to_try_to_replace, language, word_count):
    enchant_dict = enchant.Dict({"eng": "en_US", "fra": "fr_FR", "rus": "ru_RU"}[language])
    for idx in range(0, len(sub_data)):
        sub_data[idx] = (extreme_try_string_without_char(sub_data[idx][0], chars_to_try_to_replace, enchant_dict,
                                                         word_count), sub_data[idx][1])
    return sub_data
    ...
    with open("%s.srt" % os.path.splitext(mp4_path)[0], "w", encoding="utf8") as ofile:
        ...
        for data in sub_data:
            if len(data[0]) > 0:
                text = "%d\n" % idx
                text += ("%s --> %s\n" %
                         (sec_to_time(float(data[1][0]) / video_fps), sec_to_time(
                             (float(data[1][1]) / video_fps)))).replace('.', ',')
                text += data[0]
                text += "\n\n"
                ofile.write(text)
                idx += 1
    ...
    with open("%s.ass" % os.path.splitext(mp4_path)[0], "w", encoding="utf8") as ofile:
        ...
        for data in sub_data:
            if len(data[0]) > 0:
                ...
                text = data[0].replace("\n", "\\N").replace("<i>", "{\\i1}").replace("</i>", "{\\i0}").replace(
                    "<font color=\"#ffff00\">", "{\\an8}").replace("</font>", "{\\an}").replace("}{", "")
...
def strip_tags(string):
    return string.replace('\n', ' ').replace('<i>', '').replace('</i>', '').replace('<font color="#ffff00">',
                                                                                    '').replace('</font>', '')
...
def check_sub_data(sub_data):
    ...
    while idx < len(sub_data) - 1:
        if int(sub_data[idx][1][1]) >= int(sub_data[idx + 1][1][0]):
            score = 100. * difflib.SequenceMatcher(None, strip_tags(sub_data[idx][0]), strip_tags(
                sub_data[idx + 1][0])).ratio()
            ...
            if score >= args.auto_same_sub_threshold:
                ...
                sub_data[idx] = (score_lines(sub_data[idx][0], sub_data[idx + 1][0],
                                             args.lang), (sub_data[idx][1][0], sub_data[idx + 1][1][1]))
                del sub_data[idx + 1]
...
def new_filter_only(path, outputdir):
    ...
    vscmd = "\"%s\" -y -p --arg FichierSource=\"%s\" --arg dir=\"%s\" \"%s\" -" % (
        args.vapoursynth_path, os.path.abspath(path), os.path.abspath(outputdir), os.path.abspath(args.vpy))
...
def new_ocr_only(input_root_dir):
    ...
    if alt_exists:
        return ocr_one_screenlog(os.path.join(input_root_dir, "default")), [
            ...
            for (text, time) in ocr_one_screenlog(os.path.join(input_root_dir, "alt"))
        ]
    else:
        return ocr_one_screenlog(os.path.join(input_root_dir, "default")),
...
def post_process_subs(subsdata, outputdir, path):
    ...
    sub_data = sorted(sub_data, key=lambda file: int(file[1][0]))
    {
        "ass": convert_to_ass,
        "srt": convert_to_srt
    }[args.sub_format](sub_data, os.path.join(outputdir, os.path.basename(path)))
...
if __name__ == '__main__':
    ...
    argparser = configargparse.ArgumentParser(description='Filters a video and extracts subtitles as srt or ass.',
                                              prog="PythoCR")
    argparser.add_argument('--version', action='version', version='%(prog)s ' + VERSION)
    argparser.add_argument('-c', '--config', is_config_file=True, help='path to configuration file')
    argparser.add_argument('path', nargs='+', help='path to a video')
    argparser.add_argument('-l', '--lang', dest='lang', metavar='language', choices=['fra', 'eng', 'rus'], default='rus',
                           help='Select the language of the subtitles (default: fra)')
    argparser.add_argument('-wd', '--work-dir', dest='workdir', metavar='folder', type=str, default="temp",
                           help='Directory where I will put all my temporary stuff (default ./temp)')
    argparser.add_argument('-o', '--output-dir', dest='outputdir', metavar='folder', type=str, default="output",
                           help='Directory where I will put all my released stuff (default ./output)')
    argparser.add_argument('--log-level', dest='log_level', metavar='level',
                           choices=['CRITICAL', 'ERROR', 'WARNING', 'INFO',
                                    'DEBUG'], default='INFO', help='Set the logging level (default INFO)')
    argparser.add_argument('--ass-style', dest='ass_style', metavar='style', type=str, default=default_ass_style,
                           help='ASS style to use if sub-format is ass (default: Verdana 60)')
    argparser.add_argument('-rr', '--regex-replace', dest='regex_replace', metavar='path to regex-replace json',
                           type=type_regex_replace, default=[], help='List of regex/replace for automatic correction')
    argparser.add_argument('-hcr', '--heuristic-char-replace', dest='heurist_char_replace', metavar='char,replace',
                           type=type_heurist_char_replace, default=[],
                           help='List of char/replace for heuristic correction')
    argparser.add_argument('--sub-format', dest='sub_format', metavar='format', choices=['srt', 'ass'], default='srt',
                           type=str.lower, help='Set the outputed subtitles format (default: srt)')
    argparser.add_argument(
        '--mode', dest='mode', metavar='mode', choices=['full', 'filter', 'ocr'], default='full', help=
        'Set the processing mode. "filter" to only start the filtering jobs, "ocr" to process already filtered videos, "full" for both. (default: full)'
    )
    argparser.add_argument('--vpy', dest='vpy', metavar='vpy_file', type=str, default=None,
                           help='vapoursynth file to use for filtering (required for "filter only" and "full" modes')
    argparser.add_argument('--threads', dest='threads', metavar='number', type=int, default=multiprocessing.cpu_count(),
    ...
    argparser.add_argument(
        '--no-spellcheck', dest='no_spellcheck', action="store_true", help=
        'Deactivate the function which tries to replace allegedly bad characters using spellcheck (it will make the "heurist_char_replace" option of the userconfig useless)'
    )
    argparser.add_argument(
    ...
    argparser.add_argument('-d', '--delay', dest='delay', action="store_true",
                           help='Delay correction after every video is processed')
    argparser.add_argument('--tesseract-path', dest='tesseract_path', metavar='path to tesseract binary', type=str,
                           default="tesseract", help='The path to call tesseract (default: tesseract)')
    argparser.add_argument('--vapoursynth-path', dest='vapoursynth_path', metavar='path to vspipe binary', type=str,
                           default="vspipe", help='The path to call vapoursynth (default: vspipe)')
    ...
    logging.basicConfig(level={'CRITICAL': 50, 'ERROR': 40, 'WARNING': 30, 'INFO': 20, 'DEBUG': 10}[args.log_level])
    ...
    logging.debug("regex_replace list of %d is: %s" %
                  (len(args.regex_replace), "; ".join(["\"%s\" to \"%s\"" % entry for entry in args.regex_replace])))
    logging.debug(
        "heurist_char_replace list of %d is: %s" %
        (len(args.heurist_char_replace), "; ".join(["\"%s\" to \"%s\"" % entry for entry in args.heurist_char_replace])))
    init()
